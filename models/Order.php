<?php
    ...
    class Order
    {
        ...
        public static function getOrderListById($userId)
        {
            ...
            $result = $db->query('SELECT id, user_name, user_phone, date, status FROM product_order WHERE user_id = '.$userId.' ORDER BY status ');
            $ordersList = array();
            $i = 0;
            while ($row = $result->fetch()) {
                $ordersList[$i]['id'] = $row['id'];
        ...
        /**
         * Возвращает текстовое пояснение статуса заказа
         * @param int $status <p>Статус</p>
        ...
        /**
         * Редактирует заказ с  заланым id
