...
class Pagination
{
     ...
     * @param $index - ключ в URL указывающий на какой странице мы находимся
     */
    ...
    public function get()
    {
        ...
        $html = '<ul class="pagination__inner">';
        # Генерируем ссылки
        for ($page = $limits[0]; $page <= $limits[1]; $page++) {
            # Если текущая это текущая страница, ссылки нет и добавляется класс active
            if ($page == $this->current_page) {
                $links .= '<li class="pagination__item"><a href="#">' . $page . '</a></li>';
            } else {
        ...
        if (!is_null($links)) {
            ...
            # Создаём ссылку "На первую"
                $links = $this->generateHtml(1, 'Prew') . $links;
